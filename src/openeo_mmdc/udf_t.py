...
def compute_dates(cubearray: xr.DataArray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    new_t = np.unique([t + np.timedelta64(dd, 'D')
                                for t in ref_dates for dd in range(-4, 2, 1)])
    ...
    where_ref_date = np.array(np.broadcast_to(
        reference_index[:, None, None, None], (len(new_t), 1, *cubearray.shape[-2:])
    ))
